...
Use the following steps to configure persistent outfits (clothing and belt items) for NPCs.

1. Spawn an NPC with `maspawn npc_bandit_guard` (or some other NPC prefab).
2. Use a plugin like [Inventory Viewer](https://umod.org/plugins/inventory-viewer) to alter the NPC's clothing and belt items.
3. Run the command `masave` while looking at the NPC to save its outfit.

Note: Only the NPC's clothing and belt items will be saved, not main inventory items.
